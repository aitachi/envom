...
def save_to_chat_json(chat_data):
    """将聊天记录保存到chat.json文件"""
    try:
        chat_file = 'chat.json'
        ...
        print(f"保存聊天记录到chat.json失败: {str(e)}")
        save_to_file('error.log', f"保存聊天记录失败: {str(e)}")
        ...
        try:
            ...
            if encrypt_element is not None:
                ...
                if decrypted_xml:
                    ...
                    # 格式化时间戳
                    if create_time:
                        timestamp_unix = int(create_time)
                        timestamp_formatted = datetime.fromtimestamp(timestamp_unix).strftime('%Y-%m-%d %H:%M:%S')
                    else:
                    ...
                    # 只保存到chat.json，不发送消息
                    save_to_chat_json(chat_data)

                    # 记录处理成功的日志
                    print(f"已处理消息: 来自 {from_user} 的 {msg_type} 类型消息")
